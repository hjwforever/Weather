        ...
        $.fn.jalendar = function (options) {
            ...
            $this.find('.submit').on('click', function () {
                ...
                if (isAllDay === 'disabled') {
                    ...
                    addcalendar(time, title, false);
                } else {
                    time = thisDay[2] + '-' + thisDay[1] + '-' + thisDay[0] + ' 00:00:00';
                    addcalendar(time, title, true);
                }
            });
            ...
            $this.on('click', '.event-single .erase', function () {
                $('div[data-id=' + $(this).parents(".event-single").attr("data-id") + ']').animate({'height': 0}, function () {
                    ...
                    if (time === "All day") {
                        ...
                        time = date + " " + time + ":00";
                        deletecalendar(time, title, false);
                    }
                    $(this).remove();
                    calcTotalDayAgain();
                    calcScroll();
            ...
            //init event
            initCalendar = function (myobj) {
                for (var i = 0; i < myobj.length; i++) {
                    var title = myobj[i].fields.eventContent;
                    var time;
                    var date = myobj[i].fields.time.substring(0, 10).split('-');
